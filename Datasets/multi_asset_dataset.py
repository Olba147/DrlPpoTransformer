...
# -----------------------------
# Inference-only Multi-asset Dataset
# -----------------------------
class Dataset_Finance_MultiAsset_Pred(Dataset):
    """
    Builds the last seq_len window for prediction per asset.
    Returns the same keys as Dataset_Finance_Pred.
    """
    def __init__(
        self,
        root_path: str,
    ...
    def _load_asset(self, path: str):
        ...
        needed = self.seq_len + self.label_len
        if len(X) < needed:
            return None, None

        start = len(X) - needed
        return X[start:], raw_ohlcv[start:]
    ...
    def _read_and_build(self):
        for asset_id, fpath in self._iter_asset_files():
            if not os.path.exists(fpath):
                continue
            X, raw_ohlcv = self._load_asset(fpath)
            if X is None:
                continue

            self.asset_ids.append(asset_id)
            self.data_x[asset_id] = X
            self.ohlcv[asset_id] = raw_ohlcv
            self.index_registry.append((asset_id, 0))
