...
DEFAULT_JEPA_CONFIG_PATH = "configs/jepa_pretrain.json"
DEFAULT_PPO_CONFIG_PATH = "configs/ppo_jepa_train.json"

JEPA_CONFIG = load_json_config(None, DEFAULT_JEPA_CONFIG_PATH, __file__)
PPO_CONFIG = load_json_config(None, DEFAULT_PPO_CONFIG_PATH, __file__)

MODEL_NAME = PPO_CONFIG["model_name"]
PPO_CHECKPOINT_PATH = f"checkpoints/{MODEL_NAME}/ppo_4800000_steps.zip"
JEPA_CHECKPOINT_PATH = f"checkpoints/{MODEL_NAME}/jepa_step_300000.pt"
TICKER_LIST_PATH = PPO_CONFIG["paths"]["ticker_list_path"]

# ------------------------
# Dataset + Env settings
# ------------------------
EPISODE_LENGTH_STEPS = PPO_CONFIG["env"]["episode_length_steps"]
TRANSACTION_COST = PPO_CONFIG["env"]["transaction_cost"]
ALLOW_SHORT = PPO_CONFIG["env"].get("allow_short", True)
ACTION_MODE = PPO_CONFIG["env"].get("action_mode", "continuous")
INCLUDE_WEALTH = PPO_CONFIG["env"]["include_wealth"]
REWARD_SCALE = PPO_CONFIG["env"]["reward_scale"]

dataset_kwargs = {
    "root_path": PPO_CONFIG["dataset"]["root_path"],
    "data_path": PPO_CONFIG["dataset"]["data_path"],
    "start_date": PPO_CONFIG["dataset"].get("start_date"),
    "split": "val",
    "size": [PPO_CONFIG["dataset"]["context_len"], PPO_CONFIG["dataset"]["target_len"]],
    "use_time_features": PPO_CONFIG["dataset"].get("use_time_features", True),
    "rolling_window": PPO_CONFIG["dataset"]["rolling_window"],
    "train_split": PPO_CONFIG["dataset"]["train_split"],
    "test_split": PPO_CONFIG["dataset"]["test_split"],
    "regular_hours_only": PPO_CONFIG["dataset"].get("regular_hours_only", True),
    "timeframe": PPO_CONFIG["dataset"].get("timeframe", "15min"),
}

PATCH_LEN = JEPA_CONFIG["jepa_model"]["patch_len"]
PATCH_STRIDE = JEPA_CONFIG["jepa_model"]["patch_stride"]
JEPA_D_MODEL = JEPA_CONFIG["jepa_model"]["d_model"]
JEPA_N_FEATURES = JEPA_CONFIG["jepa_model"]["n_features"]
JEPA_N_TIME_FEATURES = JEPA_CONFIG["jepa_model"]["n_time_features"]
JEPA_NHEAD = JEPA_CONFIG["jepa_model"]["nhead"]
JEPA_NUM_LAYERS = JEPA_CONFIG["jepa_model"]["num_layers"]
JEPA_DIM_FF = JEPA_CONFIG["jepa_model"]["dim_ff"]
JEPA_DROPOUT = JEPA_CONFIG["jepa_model"]["dropout"]
JEPA_POOLING = JEPA_CONFIG["jepa_model"]["pooling"]
JEPA_PRED_LEN = JEPA_CONFIG["jepa_model"]["pred_len"]
EMA_TAU_MIN = JEPA_CONFIG["jepa_model"]["ema_tau_min"]
EMA_TAU_MAX = JEPA_CONFIG["jepa_model"]["ema_tau_max"]
...
def load_asset_universe_from_checkpoint(path: str) -> list | None:
    if not os.path.exists(path):
        return None
    try:
